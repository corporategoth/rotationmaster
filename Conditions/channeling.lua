...
addon:RegisterCondition("CHANNELING", {
    ...
    evaluate = function(value, cache)
        ...
        local name = addon.getCached(cache, UnitChannelInfo, value.unit)
        return name ~= nil
    end,
    print = function(_, value)
        return string.format(addon.playerize(value.unit, L["%s are currently channeling"], L["%s is currently casting"]),
...
addon:RegisterCondition("CHANNELING_SPELL", {
    ...
    print = function(_, value)
        return string.format(addon.playerize(value.unit, L["%s are currently channeling %s"], L["%s is currently casting %s"]),
...
addon:RegisterCondition("CHANNELING_REMAIN", {
    ...
    evaluate = function(value, cache)
        if not addon.getCached(cache, UnitExists, value.unit) then return false end
        local name, _, _, _, endTimeMS = addon.getCached(cache, UnitChannelInfo, value.unit)
        if name ~= nil then
    ...
    print = function(_, value)
        return addon.nullable(addon.unitsPossessive[value.unit], L["<unit>"]) .. " " ..
            addon.compareString(value.operator, L["time remaining on spell channel"], string.format(L["%s seconds"], addon.nullable(value.value)))
    end,
    widget = function(parent, spec, value)
        local top = parent:GetUserData("top")
        local root = top:GetUserData("root")
...
addon:RegisterCondition("CHANNEL_INTERRUPTABLE", {
    ...
    evaluate = function(value, cache)
        if not addon.getCached(cache, UnitExists, value.unit) then return false end
        local name, _, _, _, _, _, _, notInterruptible = addon.getCached(cache, UnitChannelInfo, value.unit)
        return name ~= nil and not notInterruptible
    end,
    print = function(_, value)
        return string.format(L["%s's channeled spell is interruptable"], addon.nullable(addon.units[value.unit], L["<unit>"]))
    end,
    widget = function(parent, spec, value)
        local top = parent:GetUserData("top")
        local root = top:GetUserData("root")
