...
-- From utils
local compare, compareString, nullable, keys, tomap, isin, cleanArray, deepcopy, getCached, isSpellOnSpec =
addon.compare, addon.compareString, addon.nullable, addon.keys, addon.tomap,
addon.isin, addon.cleanArray, addon.deepcopy, addon.getCached, addon.isSpellOnSpec
...
addon:RegisterCondition("SPELL_AVAIL", {
    ...
    evaluate = function(value, cache)
        ...
        return (enabled ~= 0 and start == 0 and duration == 0)
    end,
...
addon:RegisterCondition("SPELL_COOLDOWN", {
    ...
    evaluate = function(value, cache) -- Cooldown until the spell is available
        local start, duration, enabled = getCached(cache, GetSpellCooldown, value.spell)
        return compare(value.operator, duration, value.value)
    end,
    print = function(spec, value)
        local link
        if value.spell ~= nil then
